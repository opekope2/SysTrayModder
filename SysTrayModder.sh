...
then
    ...
    then
        ...
        sed -i 's/visualParent: root//g' $LOCAL_DIR/contents/ui/main.qml
        read -r -p "Would you like for the systemtray to come out from the left or right side fo the screen? (R/l) " response
        if [[ "$response" =~ ^([lL])$ ]]
        then
            sed -i 's/location: plasmoid.location/location: PlasmaCore.Types.LeftEdge/1' $LOCAL_DIR/contents/ui/main.qml
            sed -i '/id: dialog/ s/$/\n        x: Screen.desktopAvailableWidth/' $LOCAL_DIR/contents/ui/main.qml
        else
            sed -i 's/location: plasmoid.location/location: PlasmaCore.Types.RightEdge/1' $LOCAL_DIR/contents/ui/main.qml
            sed -i '/id: dialog/ s/$/\n        x: Screen.desktopAvailableWidth - dialog.width/' $LOCAL_DIR/contents/ui/main.qml
        fi
    ...
    rm -rf $LOCAL_DIR/contents/ui/org.kde.plasma.private.systemtray 2> /dev/null
    rm -rf $LOCAL_DIR/contents/ui/org.kde.plasma.systemtray 2> /dev/null
    #cp $SYSTEM_DIR/org.kde.plasma.private.systemtray $LOCAL_DIR/
    #cp $SYSTEM_DIR/org.kde.plasma.systemtray $LOCAL_DIR/
    sed -i '/import QtQuick.Layouts 1.12/ s/$/\nimport QtQuick.Window 2.2/' $LOCAL_DIR/contents/ui/ExpandedRepresentation.qml
    sed -i 's/Layout.minimumHeight: units.gridUnit \* 21/Layout.minimumHeight: Screen.desktopAvailableHeight/' $LOCAL_DIR/contents/ui/ExpandedRepresentation.qml
